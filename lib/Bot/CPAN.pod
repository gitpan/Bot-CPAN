...
inform the channels of new uploads to the CPAN (by default).
...
inform_channel_of_new_uploads, debug, search_max_results, policy and adminhost.
...
be padded with 3 spaces. (unknown) will be displayed if the bot cannot connect
to the data source, (unrated) will be displayed if the module has yet to
receieve any ratings, and (error) will occur if something went wrong.
...
If you expect the bot to have very recent CPAN data, be sure that CPANPLUS is
set to get indexes directly from ftp.funet.fi.
...
the prioritized events patch into the P::C::I core..
