...
Bot::CPAN::BasicBot. It will accept named parameters for: channels, server,
...
inform_channel_of_new_uploads, debug, search_max_results, and policy.
...
At the moment, you'll need to patch POE::Component::IRC 2.7 in order to get the
prioritized messaging (which is required to prevent blocking during sending
back lots of data to users).

The patch has been sent to the upstream author. The patch can be found in
etc/.
